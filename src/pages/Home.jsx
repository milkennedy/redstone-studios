...
function Hero() {
  ...
  return (
    <header ref={ref} className="relative flex min-h-[100svh] items-end overflow-hidden pb-20 pt-32">
      {/* atmosphere */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_90%_70%_at_75%_30%,rgba(181,103,60,0.12),transparent_65%)]" />
      <div className="absolute inset-0 bg-[linear-gradient(180deg,#14110f_0%,#19150f_55%,#14110f_100%)]" />

      {/* drifting blueprint */}
      <motion.svg style={{ y: blueprint }} className="absolute right-[-8%] top-1/2 h-[820px] w-[820px] -translate-y-1/2 opacity-[0.16]" viewBox="0 0 400 400" fill="none">
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 mx-auto w-full max-w-content px-6 md:px-10">
        <Reveal delay={0.1}><Kicker className="mb-8">Boutique AI Transformation Studio</Kicker></Reveal>
        <h1 className="t-hero max-w-[16ch] font-display text-cream">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.8 }} className="mt-10 grid grid-cols-1 items-end gap-8 md:grid-cols-[1.4fr_1fr]">
...
const phases = [
  ['Assess', 'Map current state and spot top opportunities.', 'We begin with a clear-eyed audit of how work actually happens — surfacing the workflows that consume the most time and return the least leverage.'],
  ['Design', 'Redesign workflows, governance, and teams.', 'We rebuild the operating model around AI-native execution: new processes, clear governance, and teams structured to capture the gains.'],
  ['Deploy', 'Implement tools, train teams, set standards.', 'We put working systems into the hands of your people, with the training and standards needed to make adoption stick.'],
  ['Scale', 'Expand across departments and iterate.', 'We extend what works across the organization, measuring impact and compounding returns department by department.'],
]
    ...
    <section className="border-y border-stone bg-ink2 px-6 py-28 md:px-10 md:py-36">
      <div className="mx-auto max-w-content">
        <div className="mb-16 max-w-3xl">
          <Reveal><Kicker className="mb-7">The Redstone Approach</Kicker></Reveal>
          ...
          <Reveal delay={0.15}>
            <p className="mt-7 text-[1.05rem] leading-relaxed text-muted">
              We are not a chatbot shop, and not traditional IT consultants. Our four-phase model ensures every engagement moves from discovery to measurable organizational change — not just tool installation.
            </p>
          </Reveal>
        </div>
...
function CTA() {
  return (
    <section className="relative overflow-hidden border-t border-stone px-6 py-32 md:px-10">
